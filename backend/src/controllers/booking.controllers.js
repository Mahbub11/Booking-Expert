  ...
  try {
    ...
    if (myRoom.room_status === 'unavailable') {
      return res.status(400).json(errorResponse(
        1,
        'FAILED',
        'Sorry! Currently your selected room is\'t available'
      ));
    }
    ...
    // set the room status pending
    const update = { room_status: 'booked' };
    const result = await Room.findOneAndUpdate(req.params.id, update)
    result.save();
...
// TODO: controller for get Booking by BookingID
exports.getBookingByBookingId = async (req, res) => {

  try {
    const getBooking = await Booking.findOne({ _id: req.params.orderId })
    .populate('room_id booking_status booking_dates')
    .populate('booking_by')
...
exports.setBookingStatus= async(req,res)=>{                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 
  try {
    ...
    const setRoomAvailable = { room_status: 'available' };
    if( req.body.booking_status==='fullfilled'){

      const booking = await Booking.findOneAndUpdate(bookingId, update)
      await Room.findOneAndUpdate(booking.room_id, setRoomAvailable)
    }else if(req.body.booking_status==='rejected'){
      const rejectedBook = { booking_status: 'rejected' };
       const booking = await Booking.findByIdAndUpdate(bookingId, rejectedBook)
       booking.save()
       await Room.findOneAndUpdate(booking.room_id, setRoomAvailable)

    }else if(req.body.booking_status==='booked'){
      const confirmBook = { booking_status: 'booked' };
      const bookRoom = { room_status: 'booked' };
      const booking = await Booking.findByIdAndUpdate(bookingId, confirmBook)
      booking.save()
      await Room.findOneAndUpdate(booking.room_id, bookRoom)
